...
import io.sweers.psync.PClassGenerator
import io.sweers.psync.PSyncPlugin
import io.sweers.psync.PSyncTask
import io.sweers.psync.PrefEntry
...
class PsyncTest {
  ...
  private static final PATH_PREFIX = WORKING_DIR.endsWith("psync") ? WORKING_DIR :
      "$WORKING_DIR/psync"
  ...
  void testBasicPrefEntry() {
    ...
    assertThat(entry.equals(entry))
    // Truth will check == under the hood first, which we want to avoid
  ...
  void testGetPrefEntriesFromFiles() {
    TestObserver<PrefEntry> o = new TestObserver<>()
    //noinspection GroovyAssignabilityCheck
    PSyncTask.getPrefEntriesFromFiles(
        Collections.singletonList(new File("$RESOURCE_PATH/prefs.xml")))
        .flatMap { List<PrefEntry> entries -> Observable.fromIterable(entries) }
        .subscribe(o)

    Deque<PrefEntry> values = new ArrayDeque<>(o.values())
    PrefEntry entry = values.poll()
    ...
    assertThat(
        entry.entryValuesGetterStmt).isEqualTo "getTextArray(R.array.request_types_entry_values)"
  ...
  public void testGeneration() {
    List<PrefEntry> entries = PSyncTask.getPrefEntriesFromFiles(
  ...
  public void testBasicConfiguration() {
    ...
    project.android.applicationVariants.all { ApplicationVariant variant ->
      ...
      assertThat(syncTask.outputDir).isEqualTo new File(
      ...
      assertThat(xmlFiles[0]).isEqualTo new File(
          "${FIXTURE_WORKING_DIR}/src/main/res/xml/prefs.xml")

      syncTask.generate(TestHelper.getTaskInputs())

      assertThat(syncTask.outputDir.exists())
      assertThat(
          "${syncTask.outputDir}/{${syncTask.packageName.replace('.', '/')}/${syncTask.className}.java")
    }
