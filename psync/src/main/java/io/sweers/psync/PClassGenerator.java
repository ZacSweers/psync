...
public final class PClassGenerator {
  ...
  private static final ClassName CN_RX_PREFERENCES =
      ClassName.get("com.f2prateek.rx.preferences", "RxSharedPreferences");
  private static final ClassName CN_RX_PREFERENCE =
      ClassName.get("com.f2prateek.rx.preferences", "Preference");
   ...
   */
  public static void generate(List<PrefEntry> inputKeys,
      String packageName,
      File outputDir,
      String className,
      boolean generateRx) throws IOException {
    TypeSpec.Builder pClass = TypeSpec.classBuilder(className)
    ...
    JavaFile javaFile = JavaFile.builder(packageName, pClass.build())
        .build();
  ...
  private static void setUpContextAndPreferences(TypeSpec.Builder pClass, boolean generateRx) {
    ...
    MethodSpec.Builder setSharedPreferencesBuilder =
        MethodSpec.methodBuilder("setSharedPreferences")
            .addModifiers(MODIFIERS)
            .addParameter(ParameterSpec.builder(SharedPreferences.class,
                "sharedPreferences",
                Modifier.FINAL)
                .build())
  ...
  private static TypeSpec generatePrefBlock(PrefEntry entry,
      String packageName,
      boolean generateRx) {
    TypeSpec.Builder entryClass = TypeSpec.classBuilder(camelCaseKey(entry.key))
        .addModifiers(MODIFIERS);
    entryClass.addField(FieldSpec.builder(String.class, "key", MODIFIERS)
        .initializer("$S", entry.key)
        .build());

    if (entry.defaultType != null) {
      if (entry.isResource) {
        entryClass.addField(FieldSpec.builder(int.class, "defaultResId", MODIFIERS)
            .initializer("$T.$N.$N",
        ...
        entryClass.addMethod(MethodSpec.methodBuilder("defaultValue")
            ...
            .addStatement(
                "return " + (isString ? "$S" : "$N"),
                isString ? entry.defaultValue : entry.defaultValue.toString())
            .build());
      }
    }
  ...
  static String camelCaseKey(String input) {
    ...
    boolean couldBeCamel = COULD_BE_CAMEL.matcher(input)
        .matches();
    if (!couldBeCamel) {
      if (input.contains("-")) {
        format = CaseFormat.LOWER_HYPHEN;
      } else if (input.contains("_")) {
        boolean isAllCaps = ALL_CAPS.matcher(input)
            .matches();
        format = isAllCaps ? CaseFormat.UPPER_UNDERSCORE : CaseFormat.LOWER_UNDERSCORE;
      }
    } else {
      format =
          Character.isUpperCase(input.charAt(0)) ? CaseFormat.UPPER_CAMEL : CaseFormat.LOWER_CAMEL;
    }
